...
function SpecialistGrid() {
  return <div className="specialist-grid">{specialists.map(({ name, detail, code }) => <div className="specialist-card" key={name}><span className="specialist-code" aria-hidden="true">{code}</span><div><p className="text-sm font-semibold">{name}</p><p className="mt-1 text-xs text-muted-foreground">{detail}</p></div></div>)}</div>
}
