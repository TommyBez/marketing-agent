    ...
    <main className="flex h-dvh min-h-[640px] bg-background p-2 md:p-3">
      <div className="flex min-w-0 flex-1 overflow-hidden rounded-2xl border bg-card shadow-2xl">
        ...
        <div className="flex min-w-0 flex-1 flex-col">
          ...
          {profile ? <Suspense fallback={<div className="flex flex-1 items-center justify-center text-sm text-muted-foreground"><span className="motion-safe:animate-pulse">Loading workspace…</span></div>}><AgentChat companyName={profile.name} /></Suspense> : <CompanyOnboarding />}
        </div>
      </div>
    </main>
  )
